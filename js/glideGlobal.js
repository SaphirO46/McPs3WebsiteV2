...
async function checkAllCheckboxesHeader ()
{
...
async function createGlideTable (headerColumnsTable, playerSearch, keysMapsRemoved, trucDeMerdeAsyncQuiFonctionnEPasLaNsm)
{
	return new Promise((resolve, reject) =>
	{
		fetch('/csv/glideLDBaerialtorti.csv')
			...
			.then(csvText =>
			{
				fetch('/csv/glideRank.json') // Charge le JSON
					.then(response =>
					{
					...
					.then(glideRankyRanky =>
					{
						...
						try
						{
							...
							if (trucDeMerdeAsyncQuiFonctionnEPasLaNsm != 1)
							{
								addClickTableRedirectParam("data-table", "Player", "/pages/glide/playerGlideProfil.html");
							}
							// Une fois tout terminé, résoudre la promesse
							resolve();
						} catch (error)
						{
							console.log(error);
							reject(error); // Rejeter si une erreur survient
						}
					})
					.catch(error =>
					{
...
// Fonction pour ajouter des boutons dans la colonne "Player"
function addClickTableRedirectParam (tableId, valueColumn, urlToRedirect)
{
	// Sélectionne le tableau
	const table = document.getElementById(tableId);

	// Trouve l'index de la colonne ayant value="Player"
	const headers = table.querySelectorAll("thead th");
	let playerColumnIndex = -1;
	headers.forEach((header, index) =>
	{
		if (header.getAttribute("value") === valueColumn)
		{
			playerColumnIndex = index;
		}
	});

	// Si la colonne "???" n'est pas trouvée, arrête
	if (playerColumnIndex === -1)
	{
		console.error("Colonne avec value='???' introuvable.");
		return;
	}

	// Parcourt les lignes du corps du tableau
	const rows = table.querySelectorAll("tbody tr");
	rows.forEach(row =>
	{
		// Récupère la cellule de la colonne "Player"
		const playerCell = row.cells[playerColumnIndex];
		const playerName = playerCell.textContent.trim();

		row.onclick = function ()
		{
			// Redirige vers l'URL avec le paramètre player
			window.location.href = urlToRedirect + '?player=' + playerName;
		};
	});
}
